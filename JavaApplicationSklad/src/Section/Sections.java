...
public class Sections extends javax.swing.JDialog {
    ...
    public Sections(java.awt.Frame parent, boolean modal, Connection c) throws SQLException {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(null);
        connection = c;

        model = new SectionModel(connection);
    ...
    private void jButtonADDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonADDActionPerformed
        NewSection np = new NewSection((Frame) this.getParent(), true, connection);
        np.setVisible(true);
        try {
            model.updateData();
    ...
    private void jButtonALTERActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonALTERActionPerformed
        if (jTable1.getSelectedRow() >= 0) {
            NewSection np = new NewSection((Frame) this.getParent(), true,
    ...
    private void jButtonDELETEActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonDELETEActionPerformed
        if (jTable1.getSelectedRow() >= 0) 
            model.delete(model.getSelectesItem(jTable1.getSelectedRow()).getNumber());
        try {
            model.updateData();
        } catch (SQLException ex) {
            Logger.getLogger(Sections.class.getName()).log(Level.SEVERE, null, ex);
