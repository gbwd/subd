...
public class Roles extends javax.swing.JDialog {
    ...
    public Roles(java.awt.Frame parent, boolean modal, Connection c) throws SQLException {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(null);
        connection = c;

        model = new RoleModel(connection);
    ...
    private void jButtonADDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonADDActionPerformed
        NewRole np = new NewRole((Frame) this.getParent(), true, connection);
        np.setVisible(true);
        try {
            model.updateData();
    ...
    private void jButtonALTERActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonALTERActionPerformed
        if (jTable1.getSelectedRow() >= 0) {
            NewRole np = new NewRole((Frame) this.getParent(), true,
