...
public class Partners extends javax.swing.JDialog {

    Connection connection;
    PartnerModel model;
    SizeSetter columnWidth;
    
    public Partners(java.awt.Frame parent, boolean modal, Connection c) throws SQLException {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(null);
        connection = c;

        model = new PartnerModel(connection);
    ...
    private void jButtonADDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonADDActionPerformed
        NewPartner np = new NewPartner((Frame) this.getParent(), true, connection);
        np.setVisible(true);
        try {
            model.updateData();
    ...
    private void jButtonALTERActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonALTERActionPerformed
        if (jTable1.getSelectedRow() >= 0) {
            NewPartner np = new NewPartner((Frame) this.getParent(), true, 
